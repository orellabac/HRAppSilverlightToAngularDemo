...
export class HomeComponent {
   @Input()
   model : Home;
   constructor (@Optional() private injectedModel : Home,private element : ElementRef <HTMLElement>) {
   }
   ngOnInit () {
      this.model = this.model || (this.injectedModel || new Home())
...
export class Home extends Page {
   ...
   public constructor () {
      ...
      this.angularComponent = HomeComponent
   }
   /**
    * 
   ...
   initControlProperties () {
      this.ContentText.Text = "Home page content";
   }
   //Partial class contents from S:\slhtml5\samples\riawalkt\C#Modified\HRApp\obj\Debug\Views\Home.g.cs
